...
ws.on('message', function message(dataStr) {
  ...
  switch(data.action) {
    case 'scan-progress':
      ...
      break;
    case 'report-created':
      console.log('Report created:', inspect(data.payload.report.results.map(r => r.violations)));
      break;
    case 'ready':
      console.log('WebSocket is ready. Rule definitions and reports are available.');
      break;
    default:
